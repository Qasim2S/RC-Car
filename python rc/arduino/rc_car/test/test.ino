...
SoftwareSerial hc06(2,3);//Create a serial connection with TX and RX on these pins
...
void setup()
{
  ...
  hc06.begin(9600);//Initialize communications with the bluetooth module
...
void loop()
{
  ...
  byte byte_count=hc06.available();//This gets the number of bytes that were sent by the python script
  if(byte_count)//If there are any bytes then deal with them
  {
    ...
    {
      inChar=hc06.read();//Read one byte
    ...
    hc06.println("Hello from Blue "+String(count));//Then send an incrmented string back to the python script
